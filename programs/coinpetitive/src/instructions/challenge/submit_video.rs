...
pub struct SubmitVideo<'info> {
    ...
    /// CHECK: Just storing submission reference
...
pub fn handle(ctx: Context<SubmitVideo>, video_url: String) -> Result<()> {
    ...
    // Transfer participation fee from participant to challenge
    ...
    // Note: The actual video content is stored off-chain, we're just 
    // recording the payment and storing a reference hash/ID
